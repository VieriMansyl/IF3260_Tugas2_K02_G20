...
const subtractVec = (a, b) => {
  return [a[0] - b[0], a[1] - b[1], a[2] - b[2]];
};

const lookAt = (cameraPos, target, up) => {
  let subtracted = subtractVec(cameraPos, target);
  let zAxis = normalizeVec(subtracted);
  let xAxis = normalizeVec(cross(up, zAxis));
  let yAxis = normalizeVec(cross(zAxis, xAxis));
